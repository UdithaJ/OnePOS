...
// "Today" for capacity = the active cashbox session's declared businessDate.
// Staff pick this when opening the session (defaults to today, but they can
// pick tomorrow for late-night opens). Falls back to the session's openedAt,
// then to wall clock, if either is missing.
async function getBusinessToday() {
  const session = await CashBoxSession.findOne({ status: 'open' }).sort({ openedAt: -1 });
  const ref = session
    ? new Date(session.businessDate || session.openedAt)
    : new Date();
